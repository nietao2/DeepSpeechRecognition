...
class PrepareTFRecords():
    ...
    def gen_am_data(self):
        # shuffle_list = [i for i in range(len(self.wav_lst))]
        wav_data_lst = []
        label_data_lst = []
        i = 0
        for wav in tqdm(self.wav_lst):
            fbank = compute_fbank(self.data_path + wav)
            pad_fbank = np.zeros((fbank.shape[0] // 8 * 8 + 8, fbank.shape[1]))
            pad_fbank[:fbank.shape[0], :] = fbank
            label = self.pny2id(self.pny_lst[i], self.am_vocab)
            i += 1
        ...
        writer = tf.python_io.TFRecordWriter(self.tfrecord_path)#, options=tf.io.TFRecordCompressionType.GZIP
        index = 0
        for wav in tqdm(self.wav_lst):
            example = tf.train.Example(
                features = tf.train.Features(
                    feature = {
                        'the_inputs':tf.train.Feature(float_list = tf.train.FloatList(value=np.array(pad_wav_data[index].flatten(), dtype=float))),
                       'the_labels':tf.train.Feature(float_list = tf.train.FloatList(value = np.array(label_data_lst[index]))),
                       'input_length':tf.train.Feature(int64_list = tf.train.Int64List(value = [input_length[index]])),
                        # 'label_length':tf.train.Feature(int64_list = tf.train.Int64List(value = [label_length[index]]))
                    }))
            serialized = example.SerializeToString()
            writer.write(serialized)
            # print(pad_wav_data[index].shape)
            index += 1
            break
        print("done")
        writer.close()
...
train_p = data_hparams()
train_p.tfrecord_path = 'data/train4.tfrecord'
train_data = PrepareTFRecords(train_p)
train_data.gen_am_data()

dev_p = data_hparams()
dev_p.tfrecord_path = 'data/dev4.tfrecord'
dev_p.data_type = 'dev'
dev_data = PrepareTFRecords(dev_p)
dev_data.am_vocab = train_data.am_vocab
dev_data.gen_am_data()

# test_p = data_hparams()
# test_p.tfrecord_path = 'data/test2.tfrecord'
# test_p.data_type = 'test'
# test_data = PrepareTFRecords(test_p)
# test_data.am_vocab = train_data.am_vocab
# test_data.gen_am_data()
